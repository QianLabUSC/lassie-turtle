...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Depth range upper bound (meters) used for clamping/mapping.",
    )
    parser.add_argument(
        "--min-depth",
        type=float,
        default=DEFAULT_MIN_DEPTH_M,
        help="Depth range lower bound (meters) used for clamping/mapping.",
    )
    parser.add_argument(
        "--preset",
    ...
    parser.add_argument(
        ...
        dest="postprocess",
        action="store_true",
        help="Enable RealSense post-processing filters (decimation/spatial/temporal/hole-filling).",
    )
    parser.add_argument(
        "--no-post",
        dest="postprocess",
        action="store_false",
        help="Disable post-processing filters (closer to RealSense Viewer defaults).",
    )
    ...
    parser.add_argument(
        ...
        dest="invert_depth",
        action="store_true",
        help="Invert depth colors so nearer pixels appear warmer (Viewer-like).",
    )
    parser.add_argument(
        "--no-invert-depth",
        dest="invert_depth",
        action="store_false",
        help="Do not invert depth colors (farther pixels appear warmer).",
    )
    parser.set_defaults(invert_depth=True)
    parser.add_argument(
        "--display-smooth",
...
def _colorize_depth(
    ...
    """Colorize depth with a *stable* mapping (no per-frame min/max normalization).

    Notes:
      - RealSense Viewer-like behavior is closer to a fixed min/max range mapping.
      - We also render invalid (0) depth as black for readability.
      - `invert=True` makes nearer pixels warmer (red) and farther pixels cooler, which
        matches the typical RealSense Viewer default feel.
    """
    # Convert requested min/max (meters) to raw depth units.
    min_units = max(min_depth_m / depth_scale, 0.0)
    max_units = max(max_depth_m / depth_scale, min_units + 1e-6)

    # Optional auto-range (percentiles) — still stable-mapped inside this detected range.
    if auto_range:
        ...
        if np.any(valid_mask):
            ...
            if high - low < 1e-6:
                high = low + 1e-3
            # Tighten the mapping range based on percentiles.
            min_units = max(min_units, float(low))
            max_units = min(max_units, float(high))

    # Fixed linear mapping to [0,255].
    depth_f = depth_image.astype(np.float32)
    depth_clipped = np.clip(depth_f, min_units, max_units)
    denom = (max_units - min_units)
    depth_norm01 = (depth_clipped - min_units) / denom  # 0=near(min), 1=far(max)

    if invert:
        depth_norm01 = 1.0 - depth_norm01  # 1=near, 0=far

    depth_u8 = (depth_norm01 * 255.0).astype(np.uint8)
    ...
    # Render invalid depth (0) as black (Viewer-like).
    invalid = depth_image == 0
    if np.any(invalid):
        colormap_img[invalid] = 0
