...
class TravelerApp(MDApp):
    ...
    def update_force_plot(self, *args):
        self.ros_node.update_force_plot(self.screen.ids.if_real_time_plot.active, self.updateplotflag)
    ...
    def check_if_start(self):
        ...
        if(self.ros_node.id == "turtle"):
            if(self.drag_traj == 5):
                self.gui_message.data.append(float(round(self.turtle_tab.ids.Slider_3.value))/1000)  # existing parameter
                self.gui_message.data.append(float(round(self.turtle_tab.ids.Slider_4.value))/1000)  # existing parameter
                self.gui_message.data.append(float(round(self.turtle_tab.ids.Slider_5.value))/1000)  # existing parameter
                self.gui_message.data.append(float(round(self.turtle_tab.ids.Slider_6.value))/1000)  # existing parameter
                self.gui_message.data.append(float(round(self.turtle_tab.ids.Slider_2.value))/1000)  # existing parameter
                self.gui_message.data.append(float(round(self.turtle_tab.ids.Slider_1.value)) * np.pi / 180)  # existing parameter
                
                # MODIFIED: Append new start and end coordinate parameters (converted to radians)
                self.gui_message.data.append(float(round(self.turtle_tab.ids.Slider_start_gamma.value)) * np.pi / 180)  # MODIFIED: start gamma
                self.gui_message.data.append(float(round(self.turtle_tab.ids.Slider_start_theta.value)) * np.pi / 180)  # MODIFIED: start theta
                self.gui_message.data.append(float(round(self.turtle_tab.ids.Slider_end_gamma.value)) * np.pi / 180)    # MODIFIED: end gamma
                self.gui_message.data.append(float(round(self.turtle_tab.ids.Slider_end_theta.value)) * np.pi / 180)    # MODIFIED: end theta
    ...
    def save_configuration(self):
        ...
        if(self.ros_node.id == "leg"):
            ...
            # MODIFIED: Append new start and end coordinate parameters to the saved configuration (in radians)
            self.gui_message.data.append(float(round(self.turtle_tab.ids.Slider_start_gamma.value)) * np.pi / 180)  # MODIFIED: saved start gamma
            self.gui_message.data.append(float(round(self.turtle_tab.ids.Slider_start_theta.value)) * np.pi / 180)  # MODIFIED: saved start theta
            self.gui_message.data.append(float(round(self.turtle_tab.ids.Slider_end_gamma.value)) * np.pi / 180)    # MODIFIED: saved end gamma
            self.gui_message.data.append(float(round(self.turtle_tab.ids.Slider_end_theta.value)) * np.pi / 180)    # MODIFIED: saved end theta
            print(self.gui_message.data)
        elif(self.ros_node.id == "MiniRhex"):
            print(112312)
            self.gui_message.data.append(float(round(self.minirhex_tab.ids.Slider_1.value)))
            self.gui_message.data.append(float(round(self.minirhex_tab.ids.Slider_2.value)))
            self.gui_message.data.append(float(round(self.minirhex_tab.ids.Slider_3.value)))
            self.gui_message.data.append(float(round(self.minirhex_tab.ids.Slider_4.value)))
            self.gui_message.data.append(float(round(self.minirhex_tab.ids.Slider_5.value)))
            self.gui_message.data.append(float(round(self.minirhex_tab.ids.Slider_6.value)))
            self.gui_message.data.append(float(round(self.minirhex_tab.ids.Slider_7.value)))
        ...
        with open(path, 'w', newline='') as f:
            ...
            if(self.ros_node.id == "turtle"):
                writer.writerow(["scenario", "real_time_plot", "lateral_angle_range", "drag_speed", "wiggle_time", "servo_speed", "extraction_angle", "wiggle frequency",
                                 "insertion_angle", "wiggle_amplitude", "start_gamma", "start_theta", "end_gamma", "end_theta"])  # MODIFIED: New fields for start/end coordinates
                writer.writerow([self.ros_node.id, self.screen.ids.if_real_time_plot.active, self.gui_message.data[0], self.gui_message.data[1], self.gui_message.data[2], self.gui_message.data[3], self.gui_message.data[4], self.gui_message.data[5],
                                 self.gui_message.data[6], self.gui_message.data[7], self.gui_message.data[8], self.gui_message.data[9],
                                 self.gui_message.data[10], self.gui_message.data[11]])  # MODIFIED: Adjust indices as needed
