...
bool traveler_msgs__msg__traveler_config__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  ...
  {  // drag_traj
    PyObject * field = PyObject_GetAttrString(_pymsg, "drag_traj");
    if (!field) {
      return false;
    }
    assert(PyFloat_Check(field));
    ros_message->drag_traj = (float)PyFloat_AS_DOUBLE(field);
    Py_DECREF(field);
  }
  {  // extrude_speed
...
PyObject * traveler_msgs__msg__traveler_config__convert_to_py(void * raw_ros_message)
{
  ...
  {  // drag_traj
    PyObject * field = NULL;
    field = PyFloat_FromDouble(ros_message->drag_traj);
    {
      int rc = PyObject_SetAttrString(_pymessage, "drag_traj", field);
      Py_DECREF(field);
      if (rc) {
        return NULL;
      }
